//Copyright 2022, Bradley Peterson, Weber State University, all rights reserved. (11/2022)
#include <cstdio>
#include <random>
#include <iostream>
#include <vector>
...
void step1() {
    ...
    unsigned int rangePerBucket;
    rangePerBucket = UINTMAX / numBuckets + 1;
    ...
    for (unsigned int i = 0; i < arrSize; i++) {
        if (numBuckets == 1) {
            ...
            currentBucket = arr[i] / rangePerBucket;
            buckets[currentBucket].push_back(arr[i]);
        }
    }
}
void step2() {

  // Start single threaded before you attempt multithreading
  // For single threaded, create a for loop that iterates numBucket times and sorts each bucket
  ...
  // For mulithreaded, use a thread pool as outlined in the instructions.
    if (numThreads > 1) {
        ...
        while (true) {
            ...
            sortOneVector(buckets[bucketIndex]);
        }
    }
}
void step3() {
...
void bucketSort() {
  ...
  //step2();
  // TODO:
  // For multi threaded, comment out the prior step() call with the parent thread
  // create array of thread trackers
  // launch/fork child threads on step2, the goal is a bunch of "thread(step2)" call
